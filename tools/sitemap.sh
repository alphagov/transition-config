...
if [ $subdir ]
	...
	echo "Spidering a subdirectory $domain/$subdir"
		wget --no-parent -I $subdir -r -l0 -x -k --reject=pdf,PDF,jpg,JPG,gif,GIF,png,PNG,doc,DOC,docx,DOCX,xls,XLS,xlsx,XLSX,ppt,PPT,pptx,PPTX -t2 -nd -P $domain.$subdir -e robots=off -o $domain.$subdir.log $target
    cat $domain.$subdir/* >> $domain.$subdir.log
		...
		wget --no-parent -r -l0 -x -k --reject=pdf,PDF,jpg,JPG,gif,GIF,png,PNG,doc,DOC,docx,DOCX,xls,XLS,xlsx,XLSX,ppt,PPT,pptx,PPTX -t2 -nd -P $domain -e robots=off -o $domain.log $target
