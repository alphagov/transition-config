...
class TransitionConfigSiteTest < MiniTest::Unit::TestCase
  ...
  def test_all_slugs_with_extra_organisation_slugs
    ...
    expected_slugs = %w[department-for-business-innovation-skills
  ...
  def test_all_slugs_with_only_whitehall_slug
    ago = TransitionConfig::Site.from_yaml(slug_check_site_filename("ago"))
    assert_equal %w[attorney-generals-office], ago.all_slugs
  end
  ...
  def test_missing_slugs
    ...
    bis = TransitionConfig::Site.from_yaml(slug_check_site_filename("bis"))
    assert_equal %w[made-up-slug], bis.missing_slugs
  end
  ...
  def test_checks_all_slugs
    ...
    assert_equal %w[non-existent-slug], exception.missing["nonexistent"]
    assert_equal %w[made-up-slug], exception.missing["bis"]
  end
  ...
  def test_checks_required_fields_are_present
    ...
    expected = {
      "bis" => %w[whitehall_slug host tna_timestamp homepage],
      "ccs" => %w[site],
    }
  ...
  def test_site_creates_yaml_when_slug_exists
    ...
    begin
      yaml = YAML.safe_load(File.read(site.filename))
      ...
      assert_equal 20_150_423_114_915, yaml["tna_timestamp"]
    ensure
